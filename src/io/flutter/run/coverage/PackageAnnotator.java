...
// com.intellij.coverage.PackageAnnotator
public class PackageAnnotator {
  ...
  public void annotate(final CoverageSuitesBundle suite, Annotator annotator) {
    ...
    //  if (coverageSuite instanceof FlutterCoverageSuite && ((FlutterCoverageSuite)coverageSuite).isPackageFiltered(qualifiedName)) {
    //    filtered = true;
    //    break;
    //  }
    //
    ...
    final Map<String, PackageCoverageInfo> packageCoverageMap = new HashMap<>();
    final Map<String, PackageCoverageInfo> flattenPackageCoverageMap = new HashMap<>();
    for (final Module module : modules) {
      ...
      if (productionRoots != null) {
        final Map<VirtualFile, DirCoverageInfo> dirsMap = new HashMap<>();
        final RootWalker rootWalker = new RootWalker(packageCoverageMap, dirsMap, flattenPackageCoverageMap, annotator, data, suite,
                                                     GlobalSearchScope.moduleScope(module));
        for (VirtualFile output : productionRoots) {
          productionRootsSet.add(output);
          final File outputRoot = findRelativeFile(rootPackageVMName, output);
  ...
  File findRelativeFile(@NotNull String rootPackageVMName, VirtualFile output) {
    ...
    if (module != null) {
      ...
      if (outputPath != null) {
        ...
        if (packageRoot.exists()) {
          final Map<String, ClassCoverageInfo> toplevelClassCoverage = new HashMap<>();
            ...
            for (File child : files) {
              if (isClassFile(child)) {
                ...
                if (toplevelClassSrcFQName.equals(qualifiedName)) {
                  collectClassCoverageInformation(child, psiClass, new PackageCoverageInfo(), data, toplevelClassCoverage,
    ...
    private void collectCoverageDataInRoot(final File packageOutputRoot,
      ...
      for (File child : children) {
        if (child.isDirectory()) {
          ...
          final VirtualFile[] childSourceRoots = Arrays.stream(sourceRoots)
          ...
          for (int i = 0; i < sourceRoots.length; i++) {
            final VirtualFile childRoot = childSourceRoots[i];
            final PackageCoverageInfo coverageInfo = childRoot != null ? dirsCoverageMap.get(childRoot) : null;
            if (coverageInfo != null) {
              dirsCoverageMap.computeIfAbsent(sourceRoots[i], DirCoverageInfo::new)
                .append(coverageInfo);
            }
          }
        }
          ...
          if (isClassFile(child)) {
            ...
            final Boolean isInSource = DumbService.getInstance(myProject).runReadActionInSmartMode(() -> {
              ...
              final PsiElement element = aClass.getNavigationElement();
            ...
            if (isInSource != null && isInSource.booleanValue()) {
              final VirtualFile virtualFile = containingFileRef.get();
              final PackageCoverageInfo coverageInfoForClass;
              if (virtualFile != null) {
      ...
      final PackageCoverageInfo flattenPackageCoverageInfo =
        flattenPackageCoverageMap.computeIfAbsent(packageVMName, k -> new PackageCoverageInfo());
      ...
      final PackageCoverageInfo packageCoverageInfo = packageCoverageMap.computeIfAbsent(packageVMName, k -> new PackageCoverageInfo());
  ...
  private void collectClassCoverageInformation(final File classFile,
    ...
    if (classData != null && classData.getLines() != null) {
      ...
      for (final String nameAndSig : methodSigs) {
        final int covered = classData.getStatus(nameAndSig);
    ...
    final ClassCoverageInfo classCoverageInfo = getOrCreateClassCoverageInfo(toplevelClassCoverage, toplevelClassSrcFQName);
  ...
  private static boolean hasGeneratedOrEmptyPrivateConstructor(@NotNull final PsiClass aClass,
    ...
    return ReadAction.compute(() -> {
      ...
      if (privateEmpty && constructors.length == 1 && constructors[0].hasModifierProperty(PsiModifier.PRIVATE)) {
        final PsiCodeBlock body = constructors[0].getBody();
  ...
  private boolean collectNonCoveredClassInfo(final File classFile,
    ...
    return SourceLineCounterUtil
      .collectNonCoveredClassInfo(classCoverageInfo, packageCoverageInfo, content, coverageSuite.isTracingEnabled(),
                                  ...
                                  //? description -> !isGeneratedDefaultConstructor(psiClass, description, myIgnoreImplicitConstructor,
                                  //                                                myIgnoreEmptyPrivateConstructors) :
                                  Conditions
                                    .alwaysTrue());
  }
